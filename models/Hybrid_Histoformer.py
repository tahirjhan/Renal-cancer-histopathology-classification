...
def conv_block(inputs, num_filters=None):
  ...
  #  x3 = tf.keras.layers.Conv2D(filters=num_filters, kernel_size=(3, 3), dilation_rate=4,activation='relu', padding='same')(x1)

    x = Concatenate()([x1, x2])
   # x = tf.keras.layers.Conv2D(filters=num_filters, kernel_size=(1, 1), activation='relu', padding='same')(x)
  #  x = Add()([x, x1])
...
#---------------------------------------------------------------------------
#dim = [144, 192, 240]
def RCG_vit(inputs, num_filters, dim, patch_size=2, num_layers=1):
...
def MViT(input_shape, num_channels, dim, expansion_ratio, num_layers=[2, 4, 3], num_classes=5):
    ## Input layer
    ...
    x = RCG_vit(x, num_channels[6], dim[0], num_layers=num_layers[0])
    x = RCG_vit(x, num_channels[8], dim[1], num_layers=num_layers[1])
